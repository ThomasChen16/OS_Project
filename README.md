...
To run or change the values in the code go to the bottom of the code to until def main(), then look for "processes =" and change/add "Process()" making sure to
include the following in the parantheses of the Process: Plane ID (string only; ex. P1), Plane name (string only; ex. P101), arrival time (numbers only; ex. 2), burst time (numbers only; ex. 3) and fuel percentage (can only be 100 or under; ex. fuel=50).

Full example of what "processes =" looks like and what chould be inside Process() should look like: 
